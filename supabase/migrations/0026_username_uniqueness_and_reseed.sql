-- Fixes duplicate bot usernames, confirmed both by direct observation (live
-- leaderboard showing repeats) and by the math: `countries.username` has
-- never had a uniqueness constraint, and both scripts/seed-bots.ts and
-- 0024_reseed_bots.sql generate 270 usernames independently per bot with no
-- collision check. At that volume the ~25-entry funny-username pool
-- guarantees a repeat by pigeonhole alone.
--
-- Order of operations matters here and is why this is one migration rather
-- than two: a unique index can't be created while duplicates already exist
-- in the table, so this (1) reseeds every bot with in-batch collision
-- avoidance, (2) defensively renames any bot whose new username still
-- collides with an existing REAL player's username (bots always yield -
-- real players are never touched), THEN (3) adds the unique index, and
-- (4) extends is_username_available() to also check countries, so a real
-- player can no longer register/rename into a username a bot already holds
-- either. Safe to re-run.
...
begin
  ...
  for i in 1..v_visible_count loop
    ...
    loop
      ...
      exit when not (lower(v_username) = any(v_used_lower)) or v_attempt > 50;
    end loop;
    if lower(v_username) = any(v_used_lower) then
      v_username := v_username || '_' || i::text;
    end if;
    v_used_lower := array_append(v_used_lower, lower(v_username));
  ...
  for i in 1..v_hidden_count loop
    ...
    if lower(v_username) = any(v_used_lower) then
      v_username := v_username || '_' || (v_visible_count + i)::text;
    end if;
    v_used_lower := array_append(v_used_lower, lower(v_username));
  ...
  -- Defensive: if any freshly-seeded bot username collides (case-insensitive)
  -- with an existing REAL player's username, the bot yields - real players
  -- are never renamed. This can only matter for a real player whose name
  -- happens to match a newly-rolled bot name; guarantees the unique index
  -- below can always be created.
  update countries b
  set username = b.username || '_' || substr(b.id::text, 1, 6)
  where b.is_bot
    and exists (
      select 1 from countries r
      where not r.is_bot and r.username is not null
        and lower(r.username) = lower(b.username)
    );
end $$;

-- Case-insensitive uniqueness backstop at the DB level, covering both bots
-- and real players (countries.username is kept in sync with profiles.username
-- for real players via the existing trigger).
create unique index if not exists countries_username_unique_idx
  on countries (lower(username))
  where username is not null;

-- is_username_available() previously only checked profiles, so it couldn't
-- catch a real player colliding with a bot's username. Now checks both.
...
as $$
  select not exists (select 1 from profiles where lower(username) = lower(p_username))
     and not exists (select 1 from countries where lower(username) = lower(p_username));
$$;
